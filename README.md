...
├── investment-admin-frontend/      # 管理后台前端（Vue 3 + Vite，CSR）
├── investment-admin-backend/       # 管理后台后端（FastAPI + SQLAlchemy）
├── investment-web-frontend/        # 用户端前端（Next.js 16，SSR）
└── investment-web-backend/         # 用户端 BFF 后端（NestJS + TypeScript）
```
...
| 子模块 | 技术栈 | 说明 |
|--------|--------|------|
| investment-admin-frontend | Vue 3 + Vite | 管理后台，CSR 模式 |
| investment-admin-backend | FastAPI + Python 3.12 | 管理后台后端，DDD 架构，Casdoor BFF 认证 |
| investment-web-frontend | Next.js 16 + shadcn/ui | 用户端前端，SSR 模式，Casdoor BFF 认证 |
| investment-web-backend | NestJS + TypeScript | 用户端 BFF 后端，Casdoor OIDC 对接 |
...
```
管理端：investment-admin-frontend  +  investment-admin-backend
用户端：investment-web-frontend    +  investment-web-backend
```

BFF 认证说明：
- 当前建议采用「后端 BFF 统一对接 Casdoor」模式：
  - `investment-web-frontend` 跳转到 `investment-web-backend /auth/login`
  - `investment-admin-frontend` 跳转到 `investment-admin-backend /auth/login`
...
### 1) `investment-web-backend`（NestJS）
...
### 2) `investment-admin-backend`（FastAPI）
...
2. 回到**父仓库根目录**（如 `investment-app`）：`git add` → `git commit` → `git push`，把子模块目录在父仓库里记录的**提交指针**一并推上去。
...
| 场景 | 常用命令 |
|------|----------|
| 只把「某子模块」更新后的指针写进父仓库 | `git add investment-admin-frontend`（目录名即 `.gitmodules` 里的 `path`） |
...
注意：子模块**内部的**源码改动，只能在**进入该子模块目录后**用 `git add` / `commit`，不要在父仓库里对 `investment-xxx/src/...` 单独 `add`。
...
**命令示例**（假设父仓库根目录名为 `investment-app`）：

```bash
# 例 1：只更新「管理端前端」子模块指针（子模块里已 commit + push 完毕）
cd investment-app
git add investment-admin-frontend
git commit -m "chore: bump investment-admin-frontend"
git push origin master
...
```bash
git clone --recurse-submodules https://gitee.com/sunmoonlion/investment-app.git
```

**后续更新**（本地已有父仓库，只同步远程最新代码时）：

```bash
cd <父仓库根目录>   # 例如 investment-app
git pull
...
当 `investment-app` 中任一 backend（如 `investment-admin-backend` / `investment-web-backend`）需要数据库时，使用：

- 业务侧脚手架：
  - `investment-admin-backend/db-access-bootstrap/`
  - `investment-web-backend/db-access-bootstrap/`
- 底层开通能力（**各 backend 目录内各有一份，互不同步；与 `k8s` 仓库解耦**）：
  - `investment-admin-backend/db-provisioner/bin/dbctl`
  - `investment-web-backend/db-provisioner/bin/dbctl`
...
```bash
cd investment-admin-backend/db-access-bootstrap   # 或 investment-web-backend/db-access-bootstrap
...
- `investment-admin-backend/db-access-bootstrap/config/postgresql.k8s.env`
- `investment-web-backend/db-access-bootstrap/config/redis.k8s.env`

### 7) Backend 平台数据能力

两个 Backend 默认完整具备数据库、对象存储和 Elasticsearch 接入能力：

- `investment-admin-backend/storage-access-bootstrap/`
- `investment-web-backend/storage-access-bootstrap/`
...
```bash
cd investment-web-backend/storage-access-bootstrap
...
```bash
./k8s-scaffold/scaffold.sh investment-web-backend 8000 \
  --type backend
```
...
```bash
git clone --recurse-submodules https://gitee.com/sunmoonlion/investment-app.git <新项目名>-app
cd <新项目名>-app
```
...
脚本会自动完成：
- 将四个子模块内所有文件中的 `investment` / `Investment` / `INVESTMENT` 替换为项目名
- 重命名四个子模块目录，并修正各子模块 `.git` 指针
- 更新 `.gitmodules` 中的远程 URL
- 若当前是 Git 克隆：同步父仓 `.git/modules/*`、各子模块的 `worktree` 与远程 URL、父仓 `.git/config` 中的子模块段，并把索引里的子模块路径从 `investment-*` 改为 `<项目名>-*`（避免仅改目录名后 `git submodule` 断裂）
