...
```bash
# 1. 进入子模块
cd investment-admin-frontend   # 或其他三个模块
...
| 子模块 | 本地镜像名 | Harbor 全名 |
|---|---|---|
| investment-admin-frontend | `investment-admin-frontend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/investment-admin-frontend:<tag>` |
| investment-admin-backend | `investment-admin-backend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/investment-admin-backend:<tag>` |
| investment-web-frontend | `investment-web-frontend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/investment-web-frontend:<tag>` |
| investment-web-backend | `investment-web-backend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/investment-web-backend:<tag>` |

**Tag 规则**：`<git-sha>`（不可变，生产使用）+ `latest`（移动引用，仅开发联调）。  
**实例化**：`investment-` 前缀由 `init.sh` 替换为业务名，同步修改各模块 `mybuild/build.conf` 中的 `IMAGE` 变量。
...
| 模块 | 镜像名变量 | Tag 变量 |
|---|---|---|
| investment-admin-frontend | `ADMIN_CSR_IMAGE` | `ADMIN_CSR_TAG` |
| investment-admin-backend | `ADMIN_BACKEND_IMAGE` | `ADMIN_BACKEND_TAG` |
| investment-web-frontend | `INVESTMENT_SSR_IMAGE` | `INVESTMENT_SSR_TAG` |
| investment-web-backend | `WEB_BACKEND_IMAGE` | `WEB_BACKEND_TAG` |
...
## 一、investment-admin-frontend（Vue 3 + Vite → nginx）

### 手动构建

```bash
cd investment-admin-frontend
...
docker build -f mybuild/Dockerfile \
  ...
  -t investment-admin-frontend:1.0.0 .
...
--destination   harbor.sunmoonai.com:30443/k8s-images/investment-admin-frontend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/investment-admin-frontend:latest
--insecure
--skip-tls-verify
```
...
## 二、investment-admin-backend（FastAPI + Python 3.12）

### 手动构建

```bash
cd investment-admin-backend

# 黄金命令
docker build -f mybuild/Dockerfile \
  --build-arg REGISTRY=harbor.sunmoonai.com:30443/k8s-images \
  -t investment-admin-backend:1.0.0 .
...
--context       <子模块根目录>
--build-arg     REGISTRY=harbor.sunmoonai.com:30443/k8s-images
--destination   harbor.sunmoonai.com:30443/k8s-images/investment-admin-backend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/investment-admin-backend:latest
--insecure
--skip-tls-verify
```
...
## 三、investment-web-frontend（Next.js 16 SSR）

### 手动构建

```bash
cd investment-web-frontend

# 黄金命令（--target 可省略，run-minimal 是最后一个 stage，默认构建）
docker build -f mybuild/Dockerfile \
  --build-arg REGISTRY=harbor.sunmoonai.com:30443/k8s-images \
  --build-arg NEXT_PUBLIC_API_URL=http://localhost:8000 \
  -t investment-web-frontend:1.0.0 .
...
| `NEXT_PUBLIC_APP_NAME` | 应用名称 | `investment` |
...
--destination   harbor.sunmoonai.com:30443/k8s-images/investment-web-frontend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/investment-web-frontend:latest
--insecure
--skip-tls-verify
```
...
## 四、investment-web-backend（NestJS + TypeScript）

### 手动构建

```bash
cd investment-web-backend

# 黄金命令（--target 可省略，run 是最后一个 stage，默认构建）
docker build -f mybuild/Dockerfile \
  --build-arg REGISTRY=harbor.sunmoonai.com:30443/k8s-images \
  -t investment-web-backend:1.0.0 .
...
--target        run
--build-arg     REGISTRY=harbor.sunmoonai.com:30443/k8s-images
--destination   harbor.sunmoonai.com:30443/k8s-images/investment-web-backend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/investment-web-backend:latest
--insecure
--skip-tls-verify
```
...
- **非 root**：investment-web-backend 运行时使用 `appuser`，其余模块按需跟进
...
□ 3. investment-admin-frontend：确认 VITE_API_URL 值，修改 build.conf
□ 4. investment-admin-backend：确认 app/uv.lock 已提交
□ 5. investment-web-frontend：确认 NEXT_PUBLIC_API_URL 值，修改 build.conf
□ 6. investment-web-backend：确认 CI 只引用主 Dockerfile
...
└── k8s-images/                         ← Harbor 项目（需提前在 Harbor UI 创建）
    ...
    ├── python:3.12-slim
    ├── nginx:stable-alpine
    ├── investment-admin-frontend:<git-sha>    ← 应用镜像
    ├── investment-admin-backend:<git-sha>
    ├── investment-web-frontend:<git-sha>
    └── investment-web-backend:<git-sha>
```
