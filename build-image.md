...
```bash
# 1. 进入子模块
cd research-admin-frontend   # 或其他三个模块
...
| 子模块 | 本地镜像名 | Harbor 全名 |
|---|---|---|
| research-admin-frontend | `research-admin-frontend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/research-admin-frontend:<tag>` |
| research-admin-backend | `research-admin-backend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/research-admin-backend:<tag>` |
| research-web-frontend | `research-web-frontend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/research-web-frontend:<tag>` |
| research-web-backend | `research-web-backend:1.0.0` | `harbor.sunmoonai.com:30443/k8s-images/research-web-backend:<tag>` |

**Tag 规则**：`<git-sha>`（不可变，生产使用）+ `latest`（移动引用，仅开发联调）。  
**实例化**：`research-` 前缀由 `init.sh` 替换为业务名，同步修改各模块 `mybuild/build.conf` 中的 `IMAGE` 变量。
...
| 模块 | 镜像名变量 | Tag 变量 |
|---|---|---|
| research-admin-frontend | `ADMIN_CSR_IMAGE` | `ADMIN_CSR_TAG` |
| research-admin-backend | `ADMIN_BACKEND_IMAGE` | `ADMIN_BACKEND_TAG` |
| research-web-frontend | `RESEARCH_SSR_IMAGE` | `RESEARCH_SSR_TAG` |
| research-web-backend | `WEB_BACKEND_IMAGE` | `WEB_BACKEND_TAG` |
...
## 一、research-admin-frontend（Vue 3 + Vite → nginx）

### 手动构建

```bash
cd research-admin-frontend
...
docker build -f mybuild/Dockerfile \
  ...
  -t research-admin-frontend:1.0.0 .
...
--destination   harbor.sunmoonai.com:30443/k8s-images/research-admin-frontend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/research-admin-frontend:latest
--insecure
--skip-tls-verify
```
...
## 二、research-admin-backend（FastAPI + Python 3.12）

### 手动构建

```bash
cd research-admin-backend
...
--context       <子模块根目录>
--build-arg     REGISTRY=harbor.sunmoonai.com:30443/k8s-images
--destination   harbor.sunmoonai.com:30443/k8s-images/research-admin-backend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/research-admin-backend:latest
--insecure
--skip-tls-verify
```
...
## 三、research-web-frontend（Next.js 16 SSR）

### 手动构建

```bash
cd research-web-frontend

# 黄金命令（--target 可省略，run-minimal 是最后一个 stage，默认构建）
docker build -f mybuild/Dockerfile \
  --build-arg REGISTRY=harbor.sunmoonai.com:30443/k8s-images \
  --build-arg NEXT_PUBLIC_API_URL=http://localhost:8000 \
  -t research-web-frontend:1.0.0 .
...
| `NEXT_PUBLIC_APP_NAME` | 应用名称 | `research` |
...
--destination   harbor.sunmoonai.com:30443/k8s-images/research-web-frontend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/research-web-frontend:latest
--insecure
--skip-tls-verify
```
...
## 四、research-web-backend（NestJS + TypeScript）

### 手动构建

```bash
cd research-web-backend

# 黄金命令（--target 可省略，run 是最后一个 stage，默认构建）
docker build -f mybuild/Dockerfile \
  --build-arg REGISTRY=harbor.sunmoonai.com:30443/k8s-images \
  -t research-web-backend:1.0.0 .
...
--target        run
--build-arg     REGISTRY=harbor.sunmoonai.com:30443/k8s-images
--destination   harbor.sunmoonai.com:30443/k8s-images/research-web-backend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/research-web-backend:latest
--insecure
--skip-tls-verify
```
...
- **非 root**：research-web-backend 运行时使用 `appuser`，其余模块按需跟进
...
□ 3. research-admin-frontend：确认 VITE_API_URL 值，修改 build.conf
□ 4. research-admin-backend：确认 app/uv.lock 已提交
□ 5. research-web-frontend：确认 NEXT_PUBLIC_API_URL 值，修改 build.conf
□ 6. research-web-backend：确认 CI 只引用主 Dockerfile
...
└── k8s-images/                         ← Harbor 项目（需提前在 Harbor UI 创建）
    ...
    ├── python:3.12-slim
    ├── nginx:stable-alpine
    ├── research-admin-frontend:<git-sha>    ← 应用镜像
    ├── research-admin-backend:<git-sha>
    ├── research-web-frontend:<git-sha>
    └── research-web-backend:<git-sha>
```
